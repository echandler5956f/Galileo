...
namespace galileo
{
    namespace legged
    {
        ...
        Eigen::MatrixXd LeggedBody::initializeInputCostWeight(Eigen::MatrixXd R_taskspace, opt::ConfigVector q0)
        {
        ...
        void LeggedBody::createGeneralDynamics()
        {
            ...
            opt::TangentVectorAD vju_general_AD = Eigen::Map<opt::TangentVectorAD>(static_cast<std::vector<opt::ADScalar>>(si->get_general_joint_velocities(cu_general)).data(), si->nvju, 1);
            opt::TangentVectorAD vju_AD = Eigen::Map<opt::TangentVectorAD>(static_cast<std::vector<opt::ADScalar>>(si->get_vju(cu)).data(), si->nvju, 1);

            auto Ag = cdata.Ag;
            const Eigen::Matrix<galileo::opt::ADScalar, 6, 6> Ab = Ag.template leftCols<6>();
            const auto Ab_inv = computeFloatingBaseCentroidalMomentumMatrixInverse(Ab);
            opt::TangentVectorAD h_AD = Eigen::Map<opt::TangentVectorAD>(static_cast<std::vector<opt::ADScalar>>(si->get_ch(cx)).data(), si->nh, 1);

            opt::TangentVectorAD vb_AD1 = Ab_inv * (mass * h_AD - Ag.rightCols(si->nvju) * vju_general_AD);
            opt::TangentVectorAD tmp_v_AD1(si->nv, 1);
            ...
            opt::TangentVectorAD vb_AD2 = Ab_inv * (mass * h_AD - Ag.rightCols(si->nvju) * vju_AD);
            opt::TangentVectorAD tmp_v_AD2(si->nv, 1);
