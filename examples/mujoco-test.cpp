...
int main(int argc, char *argv[])
{
    ...
    info.nv = model.nv;
    info.nu = 12;
    info.num_3dof_ee = ee_names.size();

    std::vector<multibody::EndEffector> ees;
    for (int i = 0; i < info.num_3dof_ee; i++)
    {
    ...
    // // run RNEA to get the desired torques which yield 0 joint acceleration and static equilibrium forces
    ...
    Eigen::Vector<double, 12> static_force = Eigen::Vector<double, 12>::Zero();
    for (size_t i = 0; i < info.num_3dof_ee; ++i)
    {
    ...
    for (size_t i = 0; i < info.num_3dof_ee; ++i)
    {
        ...
        J.block(3 * i, 0, 3, info.nu) = jac_pos.rightCols(info.nu);
    }
    ...
    control_desired = Eigen::VectorXd::Zero(info.nv + 3 * info.num_3dof_ee + info.nu);
    control_desired.head(info.nv) = Eigen::VectorXd::Zero(info.nv);
    control_desired.segment(info.nv, 3 * info.num_3dof_ee) = static_force;
    control_desired.tail(info.nu) = tau_des;
...
void loop(const mjModel *m, mjData *d)
{
    ...
    Eigen::VectorXd q_swapped = state;
    // Extract the quaternion (of size 4) starting at index 3.
    Eigen::Vector4d quat = q_swapped.segment(3, 4);
    ...
    // Replace the segment in q_swapped.
    q_swapped.segment(3, 4) = quat_swapped;

    Eigen::VectorXd tau = wbc_base.update(state_desired, control_desired, q_swapped, 0, 0.0);

    std::cout << "state: " << q.transpose() << std::endl;
    std::cout << "tau: " << tau.transpose() << std::endl;

    // // simple PD controller:
    // Eigen::VectorXd tau = Eigen::VectorXd::Zero(m->nu);
