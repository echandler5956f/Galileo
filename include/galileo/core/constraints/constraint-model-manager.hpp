...
namespace galileo
{
    ...
    namespace core
    {
        ...
        public:
            ...
            // calcAll:
            // Calls each active constraint’s calc() and moves its output buffers into the
        ...
        private:
          ...
          data_collection.g_active.segment(g_offset<Is>(), DataType::G_SIZE) = std::move(local_data.g);
          data_collection.h_active.segment(h_offset<Is>(), DataType::H_SIZE) = std::move(local_data.h); }(), 0)...};
            }
            ...
            // is set, calls the model’s calc() and moves its buffers into the output.
                                       ...
                                       ConstraintDataCollection &data_collection)
            {
                if constexpr (I < sizeof...(ConstraintModels))
                {
                    if (active_mask_ & (1ULL << I))
                    {
                        ...
                        data_collection.g_active.segment(g_offset, DataType::G_SIZE) = std::move(local_data.g);
                        data_collection.h_active.segment(h_offset, DataType::H_SIZE) = std::move(local_data.h);
