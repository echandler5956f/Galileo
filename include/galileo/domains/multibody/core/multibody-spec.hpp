#ifndef __galileo_multibody_core_multibody_spec_hpp__
#define __galileo_multibody_core_multibody_spec_hpp__
...
namespace galileo
{
        ...
        : public SystemSpecTpl<BasicSpec, AddDim_v<_NQb, _NQj>, AddDim_v<_NVb, _NVj>, AddDim_v<_NVj, _NRotors>>
    {
        ...
        using SS = SystemSpecTpl<BasicSpec, AddDim_v<_NQb, _NQj>, AddDim_v<_NVb, _NVj>, AddDim_v<_NVj, _NRotors>>;
        using Base = SS;
        using BS = BasicSpec;
        ...
        /* ---------------------------------------------------------------- */
        /* Template types, override the base spec templated types */
        /* ---------------------------------------------------------------- */
        ...
        /* Constructors */
        /* ---------------------------------------------------------------- */
        MultibodySpecTpl(int NQb_, int NQj_, int NVb_, int NVj_, int NRotors_)
            : Base(NQb_ + NQj_, NVb_ + NVj_, NVj_ + NRotors_),
              nqb_dim_{NQb_},
              nqj_dim_{NQj_},
              nvb_dim_{NVb_},
              nvj_dim_{NVj_},
              nrotors_dim_{NRotors_}
        {
        }
        MultibodySpecTpl(const RobotModel_t &robot, const std::string &root_joint_name = "root_joint")
            : Base(robot.nq, robot.nv, robot.nv - get_root<false>(robot, root_joint_name)),
              nqb_dim_{get_root<true>(robot, root_joint_name)},
              nqj_dim_{get_nq() - get_nqb()},
              nvb_dim_{get_root<false>(robot, root_joint_name)},
              nvj_dim_{get_nv() - get_nvb()},
              nrotors_dim_{0}
        {
        }
        ...
        template <bool NQorNV>
        static inline int get_root(const RobotModel_t &robot, const std::string &root_joint_name = "root_joint")
        {
            if (!robot.existJointName(root_joint_name)) return 0;

            const auto &joint = robot.joints[robot.getJointId(root_joint_name)];
            if constexpr (NQorNV)
                return joint.nq();
            else
                return joint.nv();
...
#endif // __galileo_multibody_core_multibody_spec_hpp__
