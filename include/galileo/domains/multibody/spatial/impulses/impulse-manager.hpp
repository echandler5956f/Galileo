#ifndef __galileo_multibody_spatial_impulses_impulse_manager_hpp__
#define __galileo_multibody_spatial_impulses_impulse_manager_hpp__

#include "galileo/common/container/manager-base.hpp"

#include "galileo/domains/multibody/spatial/impulses/impulse-base.hpp"
#include "galileo/domains/multibody/spatial/impulses/impulse-generic.hpp"
...
namespace galileo
{
    ...
    template <typename PhaseSpec, template <typename PS> class ImpulseCollectionTpl>
    struct ImpulseItemTpl : public ManagerItemTpl<ImpulseItemTpl<PhaseSpec, ImpulseCollectionTpl>>
    {
        using PS = PhaseSpec;
    ...
    template <typename PhaseSpec, template <typename PS> class ImpulseCollectionTpl>
    class ImpulseDataManagerTpl : public ManagerDataBase<ImpulseDataManagerTpl<PhaseSpec, ImpulseCollectionTpl>>
    {
    public:
        using PS = PhaseSpec;
    ...
    template <typename PhaseSpec, template <typename PS> class ImpulseCollectionTpl>
    class ImpulseModelManagerTpl : public ManagerModelBase<ImpulseModelManagerTpl<PhaseSpec, ImpulseCollectionTpl>>
    {
    public:
        using PS = PhaseSpec;
        ...
        ImpulseModelManagerTpl(const PS &ps, const State_t &state) : Base(), ps_(ps), state_(state) {}
        ...
        {
            typename ForceVector_t::iterator it_fext;
            for (it_fext = data.fext.begin(); it_fext != data.fext.end(); ++it_fext)
            {
                *it_fext = Force_t::Zero();
            }
                 ...
                 ++it_m, ++it_d)
            {
                ...
                if (m_i.active)
                {
                    ...
                    const JointIndex_t joint = get_state().get_robot().frames[d_i.frame()].parentJoint;
...
#endif // __galileo_multibody_spatial_impulses_impulse_manager_hpp__
