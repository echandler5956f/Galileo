#ifndef __galileo_multibody_spatial_impulses_impulse_6d_hpp__
#define __galileo_multibody_spatial_impulses_impulse_6d_hpp__
...
namespace galileo
{
    ...
    template <typename PhaseSpec>
    struct ImpulseData6dTpl : public ImpulseDataBase<ImpulseData6dTpl<PhaseSpec>, PhaseSpec>
    {
    public:
        using PS = PhaseSpec;
    ...
    template <typename PhaseSpec>
    struct ImpulseModel6dTpl : public ImpulseModelBase<ImpulseModel6dTpl<PhaseSpec>, PhaseSpec>
    {
    public:
        using PS = PhaseSpec;
        ...
        ImpulseModel6dTpl(const PS &ps, const State_t &state, const FrameIndex_t id, const ReferenceFrame_t &type)
            : Base(id, type, DimNC_t()), ps_(ps), state_(state)
        {
        }
        ...
        void calcDiff(Data_t &data, const Eigen::MatrixBase<StateVectorType> &x) const
        {
            ...
            pinocchio::getJointVelocityDerivatives(
                get_state().get_robot(), *data.robot, joint, pinocchio::LOCAL, data.v_partial_dq, data.v_partial_dv);
...
#endif // __galileo_multibody_spatial_impulses_impulse_6d_hpp__
