#ifndef __galileo_multibody_spatial_impulses_impulse_base_hpp__
#define __galileo_multibody_spatial_impulses_impulse_base_hpp__

#include "galileo/domains/multibody/spatial/force-base.hpp"
...
namespace galileo
{
    ...
    struct ImpulseBaseTpl;

    template <typename Derived, typename PhaseSpec>
    struct ImpulseDataBase;

    template <typename Derived, typename PhaseSpec>
    struct ImpulseModelBase;

    // We are basically forward propogating the responsibility of filling the traits
    // to the derived CRTP class, because ImpulseDataBase/ImpulseModelBase are CRTP base classes
    template <typename Derived, typename PhaseSpec>
    struct traits<ImpulseBaseTpl<Derived, PhaseSpec>>
    {
        using PS = PhaseSpec;
        ...
        // Forward propogating the traits of the derived class. Basically, instead of looking at ImpulseBaseTpl,
        // ForceDataBase will look at the traits of the derived class.
        GALILEO_IMPULSE_DATA_TYPEDEF(Meta_t);
    };

    template <typename Derived, typename PhaseSpec>
    struct traits<ImpulseDataBase<Derived, PhaseSpec>>
    {
        using PS = PhaseSpec;
    ...
    template <typename Derived, typename PhaseSpec>
    struct traits<ImpulseModelBase<Derived, PhaseSpec>>
    {
        using PS = PhaseSpec;
    ...
    template <typename Derived, typename PhaseSpec>
    struct ImpulseDataBase : public ForceDataBase<ImpulseDataBase<Derived, PhaseSpec>, PhaseSpec>
    {
    public:
        using PS = PhaseSpec;
        ...
        using Data_t = typename traits<Meta_t>::Data_t;
        using Model_t = typename traits<Meta_t>::Model_t;
        using Base = ForceDataBase<ImpulseDataBase<Derived, PS>, PS>;
        ...
        /** Return reference to this as derived object */
        inline const Derived &derived() const & noexcept { return *static_cast<Derived const *>(this); }
    ...
    protected:
        ...
        inline ImpulseDataBase(const ImpulseDataBase &clone) : Base(clone) { *this = clone; }
        inline ImpulseDataBase &operator=(const ImpulseDataBase &clone) { return *this; }

    }; // struct ImpulseDataBase

    template <typename Derived, typename PhaseSpec>
    struct ImpulseModelBase : public internal::CRTP<Derived>
    {
    public:
        using PS = PhaseSpec;
        ...
        void updateForceDiff(Data_t &data, const Eigen::MatrixBase<MatrixNcNdxType> &df_dx) const
        {
        ...
        void setZeroForce(Data_t &data) const { this->derived().setZeroForceImpl(data); }
        ...
        void set_id_impl(const FrameIndex_t &id) { id_ = id; }
        ReferenceFrame_t get_type() const { return this->derived().get_type_impl(); }
        ReferenceFrame_t get_type_impl() const { return type_; }
        void set_type(const ReferenceFrame_t &type) { this->derived().set_type_impl(type); }
        void set_type_impl(const ReferenceFrame_t &type) { type_ = type; }
        int get_nc() const { return this->derived().get_nc_impl(); }
        int get_nc_impl() const { return nc_dim_.value(); }
...
#endif // __galileo_multibody_spatial_impulses_impulse_base_hpp__
