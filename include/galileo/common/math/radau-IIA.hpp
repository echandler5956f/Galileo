...
namespace galileo
{
    namespace math
    {
        ...
        public:
            ...
            {
                assert(jacobi_roots_.get_alpha() == 1.0 && jacobi_roots_.get_beta() == 0.0 &&
                       "RadauIIATpl requires roots from shifted Legendre polynomials (alpha = 1.0, beta = 0.0).");
            }
            ...
            {
                assert(k >= 0 && "Degree k must be non-negative.");
